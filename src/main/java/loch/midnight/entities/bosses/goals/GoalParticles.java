...
public class GoalParticles {
    ...
    public void emmit_particles() {
        ...
        if (this.boss.getWorld() instanceof ServerWorld world) {
    ...
    public void emmit_particle_of(ParticleEffect particleType, final int frequency) {
        ...
        if (this.boss.getWorld() instanceof ServerWorld world) {
            var pos = this.boss.getPos();
            var offset = new Vec3d(random.nextDouble(), random.nextDouble(), random.nextDouble());
            this.spawn_particles(world, particleType, pos, offset, frequency, 0.3);
        }
