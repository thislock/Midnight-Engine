...
import name.thislock.Items.ChaosFabricItems;
...
public class Boss extends HostileEntity implements PolymerEntity, RangedAttackMob {
    ...
    public void createBossDrops(ServerWorld world) {
        for (int i = 0; i < random.nextBetween(50, 200); ++i)
            this.dropItem(world, ChaosFabricItems.DYNAMITE_GLOVE);
    }
    ...
    public void tick() {
        try {
            ...
            var error_message = """
                    ...
                    ERROR:   \n{""" + e.getMessage() + "}";
            ChaosFabric.announce(error_message);
    ...
    public void boss_info(String info) {
        ChaosFabric.LOGGER.info("FOR BOSS {}: {}", boss_name, info);
    }
